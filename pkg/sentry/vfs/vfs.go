...
//
// The short lock scopes check initialization against the field guards;
// they do not permit concurrent initialization or use.
//
...
//
// Preconditions: Init has succeeded. No filesystem registration, registry
// mutation, or filesystem creation may overlap Release or begin afterward.
//
// The registry remains visible during callbacks. Release borrows its map
// while quiescent; the alias is not a concurrent snapshot. Callbacks run
// without fsTypesMu held and may release other VFS references.
//
// +checklocksexclude:vfs.fsTypesMu
// +checklocksexclude:vfs.mountMu
func (vfs *VirtualFilesystem) Release(ctx context.Context) {
	// Init constructs anonMount with this VFS as its immutable owner.
	// Dropping the final mount reference may acquire vfs.mountMu.
	vfs.anonMount.DecRef(ctx)
	vfs.fsTypesMu.RLock()
	fsTypes := vfs.fsTypes
	vfs.fsTypesMu.RUnlock()
	// Keep the registry visible to callbacks without holding fsTypesMu.
...
// by vd.
//
// Releasing the mount may drop its last reference and acquire its VFS's
// mountMu. Dentry destruction is dispatched through its implementation;
// this concrete mount exclusion does not describe every destructor lock.
//
// +checklocksexclude:vd.mount.vfs.mountMu
